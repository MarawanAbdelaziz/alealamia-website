...
function WasalAmanahh() {
  ...
  function randomFun(index) {
    ...
    if (customers[index]?.wasalAmanah != null) {
      do {
        uniqueRandomNum = randomInt(1, customers[index]?.wasalAmanah.length + 2)
      } while (
  ...
  const onSubmit = (data) => {
    ...
    if (customerId != null) {
      if (customers[customerId].wasalAmanah == null) {
        ...
        customers[customerId].wasalAmanah = [...customers[customerId].wasalAmanah, newData]
      }
      setErrorCode(false)
  ...
  return (
    <div className="h-screen">
      ...
      <div className="pt-24  mx-5 text-center">
        ...
        <div
          className={`border w-fit mx-auto border-red-600 rounded-md py-2 px-4 mb-5 ${(customerId == null && errorCode) || errors?.countryName || errors?.notebookNumber ? 'block' : 'hidden'} `}
        >
