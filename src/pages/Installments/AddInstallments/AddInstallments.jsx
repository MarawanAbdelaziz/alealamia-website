...
function AddInstallments() {
    ...
    if (customers[index]?.installments != null) {
      do {
        uniqueRandomNum = randomInt(
          1,
          customers[index]?.installments.length + 2
        );
      } while (
  ...
  function calc(totalCal, index) {
    ...
    const amountPerMonth = (
      totalCal / installmentPeriods[index].period
    ).toFixed(2);
  ...
  function getInstallmentPeriods() {
    const index = installmentPeriods.findIndex(
      (installmentPeriod) =>
        installmentPeriod.period == getValues("installmentPeriod")
    );
    ...
    if (watch("downPayment")) {
      ...
      const totalCal = (
        getValues("itemPrice") * installmentPeriods[index].profitRatio
      ).toFixed(2);
      calc(totalCal, index);
    }
  }
      ...
      <div className="mt-10  mx-5 text-center">
        ...
        <form className="" onSubmit={handleSubmit(onSubmit)}>
          <div className="grid gap-5 w-[60%] ms-28 text-right">
            ...
            <input
              className={`py-2 px-3 w-[50%] me-auto focus:outline-none bg-transparent ${
                errors?.itemName && "border-red-600"
              } border rounded-md placeholder:text-white`}
            ...
            <input
              className={`py-2 px-3 w-[50%] me-auto focus:outline-none bg-transparent ${
                errors?.itemPrice && "border-red-600"
              } border rounded-md placeholder:text-white`}
            ...
            <div className="flex">
              ...
              <select
                className={`ms-4 px-2 py-1 bg-transparent border ${
                  errors?.installmentPeriod && "border-red-600"
            ...
            <h4 className="">
              المبلغ في الشهر :
              <span className="ms-1">
                {installmentPeriodId != null && !isNaN(amountPerMonth)
                  ? amountPerMonth
                  : "%0"}
              </span>
            </h4>
            ...
            <label
              ...
              className={` py-2 w-[20%] mt-5 px-2 border ${
                putInDrawers ? "border-green-500" : "border-red-500"
              } text-center cursor-pointer rounded-md me-4`}
            >
              احفظ في الدرج
            </label>
            <button
