...
function Latecomers() {
  ...
  const [latecomers, setLatecomers] = useState(
    JSON.parse(localStorage.getItem("latecomers")) || []
  );
          ...
          latecomer?.installments?.map(
            (installment) =>
              installment.installmentMonths.filter(
                (installmentMonth) =>
                  installmentMonth.month < formattedDate &&
                  !installmentMonth.payed
              ).length > 0 && (
                <div
                  dir="rtl"
                  key={latecomer.customer_id}
                  className="border my-5 mx-3 py-4 px-4"
                >
                  <div className="w-full flex mb-3">
                    <h3 className=" me-11">
                      كود العميل : {latecomer.customer_id}
                    </h3>
                  ...
                  >
                    <h3 className="mb-1 basis-[12%]">
                      كود القسط: {installment.installment_id}
                    </h3>
                    <h3 className="mb-1 basis-[25%]">
                      اسم القسط: {installment.installmentName}
                    </h3>
                    <h3 className="mb-1 basis-[25%]">
                      اسم السلعه : {installment.itemName}
                    </h3>
                    <h3 className="basis-[38%] flex">
                      ...
                      {installment.installmentMonths
                        ...
                        .map((installmentMonth) => (
                          <h3
                            key={installmentMonth.month}
                            className="w-fit pe-1 ms-2 border-l"
                          >
